...
import { Trophy, Users, Play, Medal, LayoutGrid, Clock, Volume2, UserPlus, ListOrdered, RotateCcw, Image } from 'lucide-react';
...
function BracketMatchCell({ match, onWin, onSlotDrop }) {
  ...
  const Slot = ({ slotKey, player }) => {
    ...
    return (
      <div
        ...
        className={`px-3 py-2 border-b border-slate-600 last:border-b-0 min-h-[36px] flex items-center ${
          isEmpty ? 'text-slate-500 italic cursor-pointer' : 'cursor-grab active:cursor-grabbing'
        } ${isOver ? 'ring-2 ring-amber-400 bg-slate-600' : 'bg-slate-800/80'} ${player?.id === match?.winner ? 'bg-emerald-700/40 font-semibold' : ''}`}
  ...
  return (
    <div className="rounded border border-slate-600 bg-slate-800 overflow-hidden min-w-[140px]">
      <div className="flex flex-col">
        <Slot slotKey="p1" player={match.p1} />
        <Slot slotKey="p2" player={match.p2} />
      </div>
      {!done && (
        <div className="p-2 border-t border-slate-600 bg-slate-700/50">
          {isBye ? (
            <button
              type="button"
              className="w-full py-1 rounded bg-emerald-600 hover:bg-emerald-500 text-sm"
            ...
            <div className="flex gap-1">
              <button type="button" className="flex-1 py-1 rounded bg-slate-600 hover:bg-slate-500 text-sm" onClick={() => onWin(match.id, match.p1?.id)}>Win</button>
              <button type="button" className="flex-1 py-1 rounded bg-slate-600 hover:bg-slate-500 text-sm" onClick={() => onWin(match.id, match.p2?.id)}>Win</button>
            </div>
          )}
        </div>
      )}
...
function MatchHistoryPanel({ matches, onScoreChange, getMatchLabel }) {
  ...
  return (
    <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
      <h3 className="text-sm font-bold text-slate-300 mb-3">Match results</h3>
          ...
          <tbody>
            {completed.map((m, i) => (
              <tr key={m.id} className="border-b border-slate-700/50">
                <td className="py-2 pr-4 text-slate-500 font-mono text-xs">{getMatchLabel?.(m, i) ?? m.roundLabel ?? m.id}</td>
                <td className="py-2 pr-4">{m.p1?.name ?? '—'}</td>
                <td className="py-2 pr-4">{m.p2?.name ?? '—'}</td>
                <td className="py-2 pr-4 font-semibold text-emerald-400">{getPlayer(m, m.winner)?.name ?? '—'}</td>
                <td className="py-2">
                  {onScoreChange ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        min={0}
                        placeholder="—"
                        className="w-12 bg-slate-700 border border-slate-600 rounded px-1 py-0.5 text-center text-xs"
                        value={m.p1Score ?? ''}
...
function playAlarmSound() {
  try {
    ...
    // Fallback: try to use a simple beep via speech or do nothing
  }
}
...
export default function DartTournament() {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-900 text-white font-sans overflow-hidden relative">
      {isExporting && (
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-slate-800 rounded-xl border border-slate-600 p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-bold mb-2">Reset tournament?</h3>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2 rounded-lg bg-slate-600 hover:bg-slate-500 transition"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={resetGame}
                className="flex-1 py-2 rounded-lg bg-red-600 hover:bg-red-500 transition font-medium"
              >
                Reset Game
              </button>
            </div>
      ...
      {completedMatchesForExport.length > 0 && (
        <div
          ...
          className={`w-[600px] bg-slate-800 border border-slate-600 rounded-xl p-6 text-white ${
            isExporting ? 'fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[60]' : 'fixed left-[-9999px] top-0'
          }`}
          style={{ fontFamily: 'system-ui, sans-serif' }}
        >
          <h2 className="text-xl font-bold mb-1 flex items-center gap-2">
            <Trophy className="text-yellow-400" /> Men&apos;s Dart Night — Tournament Results
          </h2>
            ...
            <tbody>
              {completedMatchesForExport.map((row, i) => (
                <tr key={i} className="border-b border-slate-700/50">
                  <td className="py-2 pr-4 text-slate-500 font-mono text-xs">{row.match}</td>
                  <td className="py-2 pr-4">{row.p1}</td>
                  <td className="py-2 pr-4">{row.p2}</td>
                  <td className="py-2 pr-4 font-semibold text-emerald-400">{row.winner}</td>
                  <td className="py-2 pr-4 text-slate-400">{row.loser}</td>
          ...
          {champion && (
            <div className="mt-4 pt-4 border-t border-slate-600 text-center">
              <span className="text-slate-400 text-sm">Champion: </span>
              <span className="text-lg font-bold text-amber-400">{champion.name}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-h-0 max-w-6xl w-full mx-auto px-4 sm:px-6 py-4 self-center">
        <header className="flex flex-wrap justify-between items-center gap-4 mb-4 border-b border-slate-700 pb-3 shrink-0">
          <h1 className="text-2xl font-bold flex items-center gap-2 shrink-0">
            <Trophy className="text-yellow-400 shrink-0" /> Men's Dart Night
          </h1>
            ...
            {(stage === 'playing' || tournamentOver) && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 text-sm transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {stage === 'setup' && (
        ...
        <div className="max-w-md w-full bg-slate-800 p-6 rounded-xl shadow-xl border border-slate-700">
          ...
          <div className="flex gap-2 mb-6">
            <input
              className="flex-1 bg-slate-700 p-2 rounded border border-slate-600 outline-none focus:border-blue-500"
              value={newPlayer}
            ...
            <button
              onClick={addPlayer}
              className="bg-blue-600 px-4 rounded hover:bg-blue-500 transition"
            >
              Add
            </button>
          </div>
          <div className="grid grid-cols-2 gap-2 mb-6 max-h-48 overflow-y-auto">
            {players.map((p) => (
              ...
              >
                {p.name}
                <span className="text-slate-500">#{String(p.id).slice(-3)}</span>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="w-full bg-green-600 py-3 rounded-lg font-bold disabled:opacity-50 hover:bg-green-500 transition shadow-lg shadow-green-900/20"
          >
      ...
      {stage === 'playing' && (
        ...
        <div className="space-y-4 w-full flex-shrink-0">
          <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                <label className="text-slate-400 text-sm">Minutes</label>
                <input
                  type="number"
                  min={0}
                  max={99}
                  ...
                  className="w-16 bg-slate-700 border border-slate-600 rounded px-2 py-1.5 text-center text-sm"
                />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-slate-400 text-sm">Seconds</label>
                <input
                  type="number"
                  min={0}
                  max={59}
              ...
              <div className="flex items-center gap-2">
                <span className={`text-5xl sm:text-6xl font-mono font-bold tabular-nums ${remainingSeconds === 0 ? 'text-red-400' : timerRunning ? 'text-amber-400' : 'text-slate-400'}`}>
              ...
              <div className="flex gap-2">
                {!timerRunning ? (
                  ...
                  <button
                    ...
                    className="bg-amber-600 hover:bg-amber-500 px-4 py-1.5 rounded text-sm font-medium"
                  >
                    Pause
                  </button>
                )}
                <button
                  type="button"
                  onClick={() => { alarmPlayedRef.current = false; setRemainingSeconds(null); setTimerRunning(false); setAlarmRinging(false); }}
                  className="bg-slate-600 hover:bg-slate-500 px-4 py-1.5 rounded text-sm"
                >
                  Reset Timer
                </button>
              </div>
          ...
          <div className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700">
            <h3 className="text-sm font-bold px-4 py-3 border-b border-slate-700 flex items-center gap-2">
              <Medal className="text-slate-400 shrink-0" /> Players ({players.filter((p) => p.id !== 'bye').length})
            </h3>
            <p className="text-slate-500 text-xs px-4 pt-2">Drag players into bracket slots to assign</p>
            <div className="p-4 space-y-4">
              <div className="flex flex-wrap gap-2">
                {players.filter((p) => p.id !== 'bye').map((p) => (
                  <span
                    ...
                    draggable
                    onDragStart={(e) => {
                      e.dataTransfer.setData(DRAG_TYPE, JSON.stringify({ player: { id: p.id, name: p.name } }));
                      e.dataTransfer.effectAllowed = 'move';
                    }}
                    className="bg-slate-700 px-3 py-1.5 rounded text-sm cursor-grab active:cursor-grabbing hover:bg-slate-600"
                  >
                    {p.name}
                  </span>
                ))}
              ...
              <div className="pt-2 border-t border-slate-600">
                <label className="text-slate-400 text-xs block mb-1.5">Add player to roster</label>
                <div className="flex gap-2">
                  <input
                    className="flex-1 bg-slate-700 p-2 rounded border border-slate-600 outline-none focus:border-blue-500 text-sm"
                    value={newPlayer}
                    onChange={(e) => setNewPlayer(e.target.value)}
                    placeholder="Name..."
                    onKeyDown={(e) => e.key === 'Enter' && addPlayer()}
                  ...
                  <button
                    onClick={addPlayer}
                    className="bg-blue-600 px-4 rounded hover:bg-blue-500 transition text-sm shrink-0"
                  >
                    Add
                  </button>
                </div>
          ...
          {tournamentPhase === 'roundRobin' && (
            <>
              <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
                <h3 className="text-sm font-bold text-amber-400 mb-2">Standings</h3>
                <div className="flex flex-wrap gap-2 mb-4">
                  {[...roundRobinStandings].sort((a, b) => (b.wins ?? 0) - (a.wins ?? 0)).map((s, i) => (
                    <span key={s.player?.id} className="bg-slate-700 px-3 py-1.5 rounded text-sm">
                      {i + 1}. {s.player?.name} — {s.wins ?? 0}W / {s.losses ?? 0}L
                    </span>
                  ))}
                </div>
                <h3 className="text-sm font-bold text-slate-300 mb-3">Matches</h3>
          ...
          {tournamentPhase === 'compass' && compassData && (
            <>
              {compassData.groups.map((g) => (
                <div key={g.id} className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
                  <h3 className="text-sm font-bold text-amber-400 mb-3">{g.label} bracket</h3>
                  <div className="flex flex-wrap gap-4">
                    {g.matches.map((m) => (
              ...
              {compassData.finals?.length > 0 && (
                <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
                  <h3 className="text-sm font-bold text-amber-400 mb-3">Finals</h3>
                  <div className="flex flex-wrap gap-4">
                    {compassData.finals.map((m) => (
                      <div key={m.id} className="flex flex-col">
                        <span className="text-slate-500 text-xs mb-1">{m.label}</span>
          ...
          {tournamentPhase === 'ladder' && ladderData && (
            <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
              <h3 className="text-sm font-bold text-amber-400 mb-3">Ladder rankings</h3>
              <p className="text-slate-400 text-sm mb-4">Challenge players above you to climb the ladder.</p>
              <div className="space-y-2">
                {ladderData.rankings.map((r, i) => (
                  <div key={r.player?.id} className="flex items-center gap-3 bg-slate-700 p-2 rounded">
                    <span className="text-slate-500 w-6">#{r.rank}</span>
          ...
          {tournamentPhase === 'pool' && (
            <>
              <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
                <h3 className="text-sm font-bold text-amber-400 mb-2">Qualified for finals (3 wins in a row)</h3>
                <div className="flex flex-wrap gap-2 min-h-[40px]">
                  {qualified.length === 0 ? (
                    <span className="text-slate-500 text-sm">No one yet</span>
                  ) : (
                    qualified.map((p, i) => (
                      <span key={p.id} className="bg-amber-500/20 border border-amber-500/50 px-3 py-1.5 rounded text-sm">
                        {i + 1}. {p.name}
                      </span>
                    ))
                  )}
              ...
              <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full overflow-x-auto">
                <h3 className="text-sm font-bold text-slate-300 mb-3 flex items-center gap-2">
                  ...
                  <div className="flex flex-col gap-8">
                    {currentRoundMatches.map((m, i) => (
                      <div key={m.id} className="flex items-center gap-2">
                        <span className="text-slate-500 text-xs w-6">M{i + 1}</span>
                        <BracketMatchCell
                          match={m}
                          onWin={handlePoolWin}
              ...
              <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full">
                <h3 className="text-sm font-bold text-slate-300 mb-2">Win streak</h3>
                <div className="flex flex-wrap gap-2">
                  {players.filter((p) => p.id !== 'bye').map((p) => (
                    <span key={p.id} className="bg-slate-700 px-3 py-1 rounded text-sm">
                      {p.name}: <span className="text-amber-400 font-mono">{getWinStreak(matchHistory, p.id)}</span> in a row
                    </span>
                  ))}
                </div>
              </div>
          ...
          {tournamentPhase === 'finals' && (
            <div className="bg-slate-800 rounded-xl border border-slate-600 p-4 w-full overflow-x-auto">
              <h3 className="text-sm font-bold text-amber-400 mb-3">Finals bracket</h3>
              <div className="flex gap-8 flex-wrap">
                {finalsMatches.map((m) => (
                  <div key={m.id} className="flex flex-col">
                    <span className="text-slate-500 text-xs mb-1">{m.type}</span>
                    <BracketMatchCell
                      match={m}
          ...
          {tournamentOver && (
            <div className="space-y-4 w-full">
              <div className="max-w-md mx-auto bg-amber-500/10 border border-amber-500/50 rounded-xl p-6 text-center">
                <h3 className="text-xl font-bold text-amber-400">Champion</h3>
                <p className="text-2xl font-black text-white mt-2">{champion?.name}</p>
              </div>
              {tournamentPhase === 'finals' && finalsMatches.length >= 2 && (
                <div className="max-w-md mx-auto bg-slate-800 border border-slate-600 rounded-xl p-4 text-center">
                  <h3 className="text-sm font-bold text-slate-400 mb-2">3rd & 4th place</h3>
